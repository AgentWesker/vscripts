...
function CoopThink()
{
	if ( m_bReadyRoomStartCheckDeploy == true && m_bReadyRoomPlayersDeployed == false )
	{
	...
	{
		EntFire( "@radiovoice","RunScriptCode", "PlayVcd( ReturnRandom(367,368,13,380,0,0) )", 0 );
...
function ImogenCalloutBomb01()
{
	...
	m_flPlantFirstBombNag = Time() + RandomInt(14, 20);
}
...
function SpawnEnemiesM3( nGate )
{
	...
	else if ( nGate == 1 )
	{
		...
		EntFire( "@coopmission_manager_script", "RunScriptCode", "SpawnNextWave( 4 )", 9 );

		EntFire( "@spawn_ct_maingate", "SetDisabled", "", 10 ); 
		EntFire( "@spawn_ct_m3_forest_1", "SetEnabled", "", 11 );  

		SendToConsoleServer( "mp_randomspawn_los 1" );
		SendToConsoleServer( "mp_randomspawn_dist 1800" );
		SendToConsoleServer( "mp_use_respawn_waves 1" );
...
{
	EntFire( "trk_fake_chopper_1", "StartForward", "", 25 );		
	EntFire( "Chopper_arrive_m1", "Enable", "", 26 );
	EntFire( "Chopper_arrive_m1", "SetAnimation", "helicopter_coop_hostagepickup_flyin", 25 );			
	EntFire( "Chopper_arrive_m1", "SetDefaultAnimation", "helicopter_coop_hostagepickup_idle", 40 );

	// 30 seconds!
	EntFire( "@radiovoice","RunScriptCode", "PlayVcd(32)", 10 );
	EntFire ( "@radiovoice","RunScriptCode", "PlayVcd(370)", 12.5 );	

	// 15 seconds!
	EntFire( "@radiovoice","RunScriptCode", "PlayVcd(33)", 23 );
	// 5 seconds!
	//EntFire( "@radiovoice","RunScriptCode", "PlayVcd(34)", 33 );

	EntFire( "prt_copterroto_pickup_m1", "Stop", "", 36 );	
	EntFire( "prt_copterroto_pickup_m1", "Start", "", 37 );	
	 
	EntFire( "helicopter_sound_pickup", "SetParent", "trn_fake_chopper_m1", 0 );
	//EntFire( "helicopter_sound_pickup", "SetParentAttachment", "strobe_red", 1 ); 
	EntFire( "helicopter_pickup_sound", "PlaySound", "", 23 );
	EntFire( "trn_fake_chopper_m1", "StartForward", "", 25 );
